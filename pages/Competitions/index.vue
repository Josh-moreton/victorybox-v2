<template>
  ...
  <v-container class="py-16">
    <v-row>
      <v-col cols="12">
        <h2 class="text-h4 mb-6">Available competitions</h2>
      </v-col>
    </v-row>
      ...
      <v-col v-for="product in products" :key="product.id" cols="12" sm="6" md="4" lg="3">
        <v-card class="mx-auto d-flex flex-column" height="100%" hover>
          ...
          <v-card-actions class="mt-auto flex-column">
            ...
            <v-progress-linear :model-value="product.soldPercentage" :color="product.soldPercentage > 75 ? 'deep-orange' :
              product.soldPercentage > 50 ? 'lime' :
                product.soldPercentage > 25 ? 'light-green-darken-4' :
                  'light-blue'" height="10" class="w-100 mb-4" striped rounded>
            </v-progress-linear>

            <v-btn variant="elevated" color="primary" class="snipcart-add-item w-100" :data-item-id="product.id"
              :data-item-name="product.title" :data-item-price="product.price" :data-item-url="productUrl(product.id)"
              :data-item-description="product.Description" :data-item-image="product.image" prepend-icon="mdi-ticket">
