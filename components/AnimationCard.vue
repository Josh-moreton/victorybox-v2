...
import { Bodies, Bounds, Engine, Events, Mouse, MouseConstraint, Render, World, Runner } from "matter-js";
import { ref } from "vue";

const canvasRef = ref<HTMLDivElement>();
const instance = ref<any | Render>();
let engine: any = null;
onMounted(() => {
  const containerElement = canvasRef.value!;
  const containerWidth = containerElement.clientWidth;
  const containerHeight = containerElement.clientHeight;
  engine = Engine.create();

  if (canvasRef.value && !instance.value) {
    instance.value = Render.create({
  ...
  Render.run(instance.value);

  // add mouse control
  const mouse = Mouse.create(instance.value.canvas),
...
onUnmounted(() => {
  Render.stop(instance.value);
  Engine.clear(engine);
});
</script>

<template>
  <div ref="canvasRef" class="tags-container relative z-40 h-full w-full bg-transparent"></div>
</template>
