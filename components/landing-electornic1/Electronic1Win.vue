...
<template>
  <section class="current-lottery current-lotteryv9 winbg pt-120 pb-120">
    <div class="container">
      <!-- {/* Section Header */} -->
          ...
          <NuxtLink to="/contest" class="cmn__collection radius-circle s1-bg d-center position-relative ms-lg-auto">
            <span class="cmn-cont-box text-center position-relative">
              <span class="icon mb-1">
                <PhArrowUpRight weight="bold" class="ph-bold ph-arrow-up-right n0-clr fs-three" />
              </span>
      ...
      <!-- {/* win lottery body */} -->
      <div class="row g-6">
        <ElectronicCard v-for="contest in electronic1contests" :key="`contest-${contest.id}`" v-bind="{ ...contest }" />
      </div>
      <!-- {/* win lottery body */} -->
    </div>
  </section>
